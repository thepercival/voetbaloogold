	...
	public function draw($p_oPoolUser, $p_oPool, $oPoolUsers)
	{
		...
		foreach ($oRounds as $oRound) {
			...
			foreach ($oRoundBetConfigs as $oRoundBetConfig) {
				...
				$arrLineColors = array(
					"l" => $this->m_sInnerBorder,
					"t" => $this->m_sInnerBorder,
					"r" => $this->m_sInnerBorder
				);
				$sRoundName = strip_tags($oRound->getDisplayName());
				$sRoundName = html_entity_decode($sRoundName, ENT_NOQUOTES | ENT_HTML5, 'UTF-8');
				if (trim($sRoundName) === '')
					$sRoundName = 'winnaar';
				$this->setFillColor(new Zend_Pdf_Color_Html("white"));
				$nXPos = $this->drawCell($sRoundName . " (" . $sPointsDescription . ")", $nXPos, $nYPos, $nBaseWidth - $arrColumns["uitslag"], $nRowHeight, ZendExt_Pdf_Page::ALIGNLEFT, $arrLineColors);
				...
				if ($oGames->count() > 0) {
					...
					foreach ($oGames as $oGame) {
						...
						foreach ($oPoolUsers as $oPoolUser) {
							$nPoints = 0;
							if (
								$oRoundBetConfig->getBetType() === VoetbalOog_Bet_Score::$nId
		...
		foreach ($oRounds as $oRound) {
			...
			foreach ($oRoundBetConfigs as $oRoundBetConfig) {
				...
				if (
					$oRoundBetConfig->getBetType() === VoetbalOog_Bet_Result::$nId
				...
				$sRoundNameForPoints = strip_tags($oRound->getDisplayName());
				$sRoundNameForPoints = html_entity_decode($sRoundNameForPoints, ENT_NOQUOTES | ENT_HTML5, 'UTF-8');
				$nXPos = $this->drawCell("punten " . $sRoundNameForPoints . " ( " . $sRBCDescription . " )", $nXPos, $nYPos, $nBaseWidth, $nRowHeight, ZendExt_Pdf_Page::ALIGNRIGHT, $arrLineColors);
				foreach ($oPoolUsers as $oPoolUser) {
					$sText = "";
					if ($oRound->getState() === Voetbal_Factory::STATE_PLAYED)
		...
		$arrLineColors = array(
			"l" => $this->m_sOuterBorder,
			"t" => $this->m_sOuterBorder,
			"r" => $this->m_sOuterBorder,
			"b" => $this->m_sOuterBorder
		);
		foreach ($oPoolUsers as $oPoolUser) {
			$sText = "";
