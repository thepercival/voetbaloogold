...
class Zend_View_Helper_ShowAllTimes
{
	...
	protected function getCompetitionSeasons($oPools)
	{
		...
		foreach ($oPools as $oPool) {
			...
			if (
				$oCompetitionSeason->getState() === Voetbal_Factory::STATE_PLAYED or
	...
	protected function getAllTimeRanking($oPool)
	{
		...
		$sHtml .= "<tr class=\"tableheader\"><th style='position:sticky;left:0;z-index:1;background:rgba(19,81,19,0.5);'>deelnemer</th>";
		foreach ($oCompetitionSeasons as $oCompetitionSeason) {
			$sHtml .= "<th style='text-align: right;'>" . $oCompetitionSeason->getCompetition()->getAbbreviation() . "<br>";
			$sHtml .= $oCompetitionSeason->getSeason()->getName() . "</th>";
		}
		$sHtml .= "<th style='text-align:right;position:sticky;right:0;z-index:1;background:rgba(19,81,19,0.5);'>tot.</th></tr>";
		...
		foreach ($arrAllTimeRankTotals as $nUserId => $nTotalPoints) {
			...
			foreach ($oCompetitionSeasons as $oCompetitionSeason) {
				...
				$sHtml .= "<td style='text-align:right'>" . $vtPoints . "</td>";
			}
			$sHtml .= "<td style='text-align:right;position:sticky;right:0;z-index:1;background:#fff;'>" . $nTotalPoints . "</td></tr>";
		}
